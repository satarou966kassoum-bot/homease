...
## 2bis. Créer ton compte Cloudinary (gratuit — pour les photos et vidéos)

Le plan gratuit d'Hébergement Render n'a pas de disque persistant : les
photos/vidéos des annonces sont donc envoyées directement depuis le
navigateur vers Cloudinary (le backend ne fait que signer la requête).

1. Va sur https://cloudinary.com et crée un compte gratuit (25 Go de
   stockage et de bande passante offerts, largement suffisant pour démarrer).
2. Sur le tableau de bord, note :
   - **Cloud name**
   - **API Key**
   - **API Secret**
3. Renseigne-les dans le `.env` du backend (`CLOUDINARY_CLOUD_NAME`,
   `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`).

Sans ces variables, la publication d'annonce fonctionne toujours, mais
l'ajout de photos/vidéos affichera une erreur ("service non configuré").
